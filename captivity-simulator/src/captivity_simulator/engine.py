...
def _deliver_pending_gifts_for_night(state: dict[str, Any]) -> list[dict[str, str]]:
    ...
    for gift in queued:
        ...
        if item_id not in INVENTORY_ITEMS or bool(inventory.get(item_id)):
            continue
        configured_at = str(gift.get("queued_at") or now_local_iso())
...
def _change_inventory_items(state: dict[str, Any], args: dict[str, Any], *, enabled: bool) -> tuple[bool, list[str]]:
    ...
    if forbidden:
        return False, [f"包含禁用项：{forbidden}"]
    if progressive_item and enabled and not bool(inventory.get(progressive_item)) and progressive_item not in queued_ids and len(secret_entries) < MIN_INVENTORY_SECRET_ENTRIES:
    ...
    if enabled:
        ...
        for item_id in items:
            label = str((INVENTORY_ITEMS.get(item_id) or {}).get("label") or item_id)
            labels.append(label)
            if item_id not in queued_ids and not bool(inventory.get(item_id)):
                continue
            ...
            if item_id == "call_bell":
                state["call_bell_voice"] = {"line": "", "revealed": False, "configured_by": "", "configured_at": ""}
            changed.append(item_id)
        state["pending_gifts"] = pending_gifts
        state["inventory"] = inventory
        state["inventory_secrets"] = inventory_secrets
    ...
    changed_labels = [
        f"《{book_title}》" if item_id == "book" and book_title else str((INVENTORY_ITEMS.get(item_id) or {}).get("label") or item_id)
        for item_id in changed
    ]
    summary = ((("今晚已发放礼物：" if delivered_now else "已加入今晚待发放：") if enabled else "收回物品：") + "、".join(changed_labels))
    _append_event(state, action, summary, phase=str(state.get("phase") or "day"), tags=["inventory", "gift" if enabled else "revoke", "out_of_band"])
