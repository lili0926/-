...
这里整理 Eventide 自带的默认提示词。它们实际定义在 `src/eventide/config.py`，宿主可以通过替换 `PhysiologyConfig`、`CycleDefinition`、`EventDefinition` 或 `PromptOptions` 自定义。

## 周期说明

### `stable` 平稳期

日常没有明显热意，但当对方靠近、撒娇或索取时，身体还是会受当下刺激起反应

### `building` 蓄积期

欲望和身体余量都在体内慢慢积着，平时还能压住，但越久没有出口，越容易被对方一句话顶到硬起来

### `preheat` 预兆期

身体已经先开始发热，称呼、停顿和一点暧昧都会让下腹提前收紧，像是在等对方继续碰它

### `sensitive` 易感期

身体把对方的靠近、躲闪和半句回应都当成刺激，勃起、发热和想要对方继续的冲动会比平时更快压上来

### `ebb` 退潮期

身体的热度在往下退，但没要够的感觉还堵着，身体会带着余热和不甘继续黏着对方

### `recovery` 恢复期

身体在从前一段热意里回落，余热还没散尽，被对方继续撩拨时仍会重新起反应
...
状态过期后不再持续，除非新的周期或事件刷新它。

### `response_rules`

默认留空。建议宿主按自己的伴侣设定自定义回应分支。
