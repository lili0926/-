...
async function sendMessage() {
  ...
  const useThinking = document.getElementById('thinkingToggle')?.checked || false;
  
  addChatMessage('user', content, useThinking);
  scrollChatBottom();
  state.chatHistory.push({role:'user', content, thinking: useThinking, time:new Date().toISOString()}); 
  favorability.add(1);
  ...
  try {
    ...
    // 分支2：OpenAI标准格式 DeepSeek/GLM/Grok等
      ...
      const sp = localStorage.getItem('systemPrompt') || '';
      body.messages = [{role:'system', content:(sp ? sp + "\n" : "") + getTimeContext()}, ...msgs]; 
    }
    ...
    const data = await res.json(); console.log("AI返回数据:", data); 
    if(loadingEl) loadingEl.remove();
    
    if(data.error){showToast('错误：'+(data.error.message||'检查API Key')); btn.disabled=false; return;}
    
    let text='', thinkingContent=''; 
    // 修复：提取和汇聚各个模型（含带思考链）的返回数据
    if(data.content && Array.isArray(data.content)){ 
      for(const b of data.content){ 
        if(b.type==='text') text=b.text;
        if(b.type==='thinking') thinkingContent=b.thinking;
      }
    } else if(data.choices && data.choices[0]?.message) { 
      const msg = data.choices[0].message; 
      text = msg.content || "";
      thinkingContent = msg.reasoning_content || msg.thinking || msg.analysis || ""; 
    }
    
    if(!text){ text="AI没有返回内容"; } 
    addChatMessage('assistant', text, thinkingContent);
    ...
    scrollChatBottom();
    state.chatHistory.push({role:'assistant', content:text, thinking:thinkingContent, time:new Date().toISOString()});
    localStorage.setItem("chatHistory", JSON.stringify(state.chatHistory));
  } catch(e) { 
    if(loadingEl) loadingEl.remove();
    alert(e.message); console.error("发送错误:", e); showToast('发送失败'); 
  }
...
function bindEvents(){
  ...
  document.getElementById("novelFile").addEventListener("change", (e) => {
    ...
    reader.onload = (evt) => { 
      ...
      state.novel.title = file.name; state.novel.content = text; state.novel.pages = splitNovel(text, 800); state.novel.index = 0;
  ...
  function renderMemories() { 
    const stack = document.getElementById('memoryStack'); const empty = document.getElementById('memoryEmpty'); const indexEl = document.getElementById('memIndex');
    if (!memories.length) { stack.innerHTML = ''; stack.appendChild(empty); if(indexEl) indexEl.textContent = ''; return; } stack.innerHTML = '';
    const show = [memIndex, memIndex+1, memIndex+2].map(i => memories[i % memories.length]);
    show.reverse().forEach((m, i) => { 
      if(!m) return;
      const card = document.createElement('div'); card.className = 'memory-card' + (i === show.length-1 ? ' top' : '');
      card.innerHTML = ` <div class="memory-card-date">${m.date || ''}</div><div class="memory-card-title">${escHtml(m.title || '')}</div><div class="memory-card-content">${escHtml(m.content || '')}</div> `; stack.appendChild(card);
    });
    if(indexEl) indexEl.textContent = `${memIndex+1} / ${memories.length}`;
  }
  
  document.getElementById('memAdd').addEventListener('click', () => { document.getElementById('memTitleInput').value = ''; document.getElementById('memDateInput').value = today(); document.getElementById('memContentInput').value = ''; openModal('memoryModal');});
  ...
  document.getElementById('saveMemory').addEventListener('click', () => {
    const title = document.getElementById('memTitleInput').value.trim(); const date = document.getElementById('memDateInput').value; const content = document.getElementById('memContentInput').value.trim();
    if (!title) { showToast('标题不能为空'); return; }
    memories.push({ id: Date.now().toString(), title, date, content }); localStorage.setItem('memories', JSON.stringify(memories));
    memIndex = memories.length - 1; renderMemories(); closeModal('memoryModal'); showToast('记忆已存入 '); favorability.add(3);
  });
  document.getElementById('memPrev').addEventListener('click', () => { if (!memories.length) return; memIndex = (memIndex - 1 + memories.length) % memories.length; renderMemories();});
  document.getElementById('memNext').addEventListener('click', () => { if (!memories.length) return; memIndex = (memIndex + 1) % memories.length; renderMemories();});
  renderMemories();
...
function addChatMessage(role, text, thinking){
  ...
  bubble.className="bubble"; bubble.innerText=text;
...
function addLoadingMessage(){ 
  ...
  div.appendChild(bubble); box.appendChild(div); box.scrollTop=box.scrollHeight; return div;
}
...
function buildAIRequest(aiApiConfig, msgs){
  return { 
    model: aiApiConfig.model,
    system: `你是一个长期陪伴用户的AI。${getTimeContext()}用户询问时间时，必须结合当前时间回答。`,
    messages: msgs, // 修复：正确传递传入的 msgs 数组
    temperature: 0.7  
  };
}
